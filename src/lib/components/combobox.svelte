...
<div {...api.getRootProps()} class="relative">
	<div {...api.getControlProps()}>
		<input
			...
			spellcheck="false"
			autocomplete="off"
			class="text-sm font-mono px-3 border rounded-lg bg-surface-50 h-10 w-full transition-colors focus:outline-none focus:border-wwise dark:bg-surface-800 disabled:opacity-50 disabled:cursor-not-allowed focus:ring-1 focus:ring-wwise/20 {api
				.value[0]
			...
			{#if filteredItems.length > 0}
				...
				>
					{#each filteredItems.slice(0, 50) as item (item.value)}
						...
						>
							<span class="text-xs font-mono" {...api.getItemTextProps({ item })}>{item.label}</span
							>
						</li>
					{/each}
				</ul>
