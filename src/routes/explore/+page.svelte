...
interface WwiseAPI {
   ...
   * @param publish - Callback function invoked on each event
   * @param options - Options object (e.g. { return: ['id', 'name'] })
   * @returns Subscription object with unsubscribe()
   */
  subscribe(topic: string, publish: (data: Record<string, any>) => void, options?: Record<string, any>): Promise<WwiseSubscription>;
